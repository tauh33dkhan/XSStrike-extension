...
def photon(seedUrl, headers, level, threadCount, delay, timeout, skipDOM):
    ...
    def rec(target):
        ...
        print('<h2><span class="GreenDisplay">+</span> Parsing: %s<h2>' % html.escape(target) ) #Modifcation 26
        ...
        if not skipDOM:
            ...
            if highlighted and clean_highlighted not in checkedDOMs:
                ...
                print('<h2>[<span class="GreenDisplay">+</span>] Potentially vulnerable objects found at: <span class="red">%s</span></h2>' % url) # Modification 28
                for line in highlighted:
                    print('<h2><span class="red">+</span> Inline: %s </h2>' % line) # Modification
