...
def singleFuzz(target, paramData, encoding, headers, delay, timeout):
    ...
    if WAF:
        print('<h2>WAF detected: <span class="GreenDisplay">%s</span></h2>' % ( WAF ))
    else:
        print('<h2> WAF Status: <span class="GreenDisplay">Offline</span></h2>') # Modification-3 : Changed logger to print and also added html tags for display
