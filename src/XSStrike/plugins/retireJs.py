...
def retireJs(url, response):
    ...
    for script in scripts:
        if script not in getVar('checkedScripts'):
            ...
            if result:
                print('<h2>[<span class="GreenDisplay">+</span>] Vulnerable component: <span class="red">' + result['component'] + ' v' + result['version'] + '</span></h2>')
                print('<h2><span class="red">+</span> Component location: <span class="Purple">%s</span></h2>' % uri)
                details = result['vulnerabilities']
                print('<h2><span class="red">+</span> Total vulnerabilities: <span class="Purple">%i</span></h2>' % len(details))
                for detail in details:
                    print('<h2><span class="red">+</span> Summary: <span class="Purple">%s</span></h2>' % ( detail['identifiers']['summary']))
                    print('<h2><span class="red">+</span> Severity: <span class="Purple">%s</span></h2>' % detail['severity'])
                    print('<h2><span class="red">+</span> CVE:  <span class="Purple">%s</span></h2>' % detail['identifiers']['CVE'][0])
